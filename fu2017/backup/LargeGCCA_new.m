...
for it=1:MaxIt
        ...
        if EXTRA ==1 % Nesterov's accelerate Gradient
            ...
            for inner_it=1:T
                if inner_it >1
                    ...
                    if L21==1 % FISTA 
                       Q{i}  = (shrinkL1Lp(Q{i},(1/Li{i})*lambda*ones(1,M),2)); 
                    elseif L11==1
                       Q{i}  = (shrinkL1Lp(Q{i},(1/Li{i})*lambda*ones(1,M),1)); 
                    end
    ...
    switch REG_TYPE
        ...
        case 'L21'
            for i=1:I
                obj_temp =(1/2)*sum(sum(((1/sqrt(L))*X{i}*Q{i}-G).^2))+ (r/2)*sum(sqrt(sum(Q{i}.^2,2))) + obj_temp;
            end
            obj(it)=sum(obj_temp);
        case 'L11'
            for i=1:I
                obj_temp =(1/2)*sum(sum(((1/sqrt(L))*X{i}*Q{i}-G).^2))+ (r/2)*sum(sum(abs(Q{i}))) + obj_temp;
            end
            obj(it)=sum(obj_temp);
    end
