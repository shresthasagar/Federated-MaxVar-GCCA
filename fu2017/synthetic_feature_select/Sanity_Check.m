...
%         M_clean = M_clean + X_clean{i}*((X_clean{i}'*X_clean{i}+r*eye(M))\X_clean{i}')
    ...
    [Q,G_1,obj1(trial,:),dist1(trial,:),St1] = LargeGCCA_new( X,K,'G_ini',G_ini,'Q_ini',Q_ini,'r',r,'algo_type','plain','Li',Li,'MaxIt',MaxIt,'Inner_it',1,'Reg_type','fro','UM',Ubeta );
    time_proposed = toc;
    
    tic
    [Q,G_2,obj2(trial,:),dist2(trial,:),St2] = LargeGCCA_new( X,K,'G_ini',G_ini,'Q_ini',Q_ini,'r',r,'algo_type','plain','Li',Li,'MaxIt',MaxIt,'Inner_it',1000,'EXTRA',1,'UM',Ubeta ,'Reg_type','fro');
    time_proposed = toc;
    EXTRA = 0;
    WZW = 0;
    tic
